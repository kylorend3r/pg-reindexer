...
use crate::config::{
    AppConfig, DEFAULT_POSTGRES_HOST, DEFAULT_POSTGRES_PORT, DEFAULT_POSTGRES_DATABASE,
...
async fn main() -> Result<()> {
    ...
    if let Some(row) = rows.first() {
        ...
        let effective_maintenance_workers = app_config.effective_maintenance_workers();
