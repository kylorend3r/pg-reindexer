...
impl Logger {
    ...
    pub fn log_index_start(&self, index_num: usize, total: usize, schema: &str, index: &str, index_type: &str) {
        self.log(LogLevel::Info, &format!(
            "[{}/{}] Starting reindex: {}.{} ({})",
            index_num + 1, total, schema, index, index_type
        ));
    }
    ...
    pub fn log_summary(&self, total: usize, successful: usize, failed: usize, duration: std::time::Duration, threads: usize) {
        self.log(LogLevel::Info, "=== REINDEX SUMMARY ===");
        self.log(LogLevel::Info, &format!("Total indexes processed: {}", total));
        self.log(LogLevel::Success, &format!("Successfully reindexed: {}", successful));
        self.log(LogLevel::Error, &format!("Failed: {}", failed));
        self.log(LogLevel::Info, &format!("Duration: {:.2?}", duration));
        self.log(LogLevel::Info, &format!("Concurrent threads used: {}", threads));
        ...
        self.log(LogLevel::Info, &format!("Success rate: {:.1}%", success_rate));
    }

    pub fn log_dry_run(&self, indexes: &[crate::IndexInfo]) {
        self.log(LogLevel::Info, "=== DRY RUN MODE ===");
        self.log(LogLevel::Info, "No indexes will be reindexed. The following commands would be executed:");
        ...
        for (i, index) in indexes.iter().enumerate() {
            ...
            self.log(LogLevel::Info, &format!("[{}/{}] {}", i + 1, indexes.len(), reindex_sql));
        }
        
        self.log(LogLevel::Info, &format!("Total indexes to reindex: {}", indexes.len()));
        self.log(LogLevel::Info, "HINT: To actually reindex, run without --dry-run flag");
    }

    pub fn log_session_parameters(&self, maintenance_work_mem_gb: u64, max_parallel_maintenance_workers: u64) {
        self.log(LogLevel::Info, &format!("Maintenance work mem: {} GB", maintenance_work_mem_gb));
        self.log(LogLevel::Info, &format!("Max parallel maintenance workers: {}", max_parallel_maintenance_workers));
    }
}
